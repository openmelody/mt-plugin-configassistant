...
sub process_file_upload {
    ...
    my ($blog_id, $blog, $fmgr, $local_file, $asset_file, $base_url,
        $asset_base_url, $relative_url, $relative_path, $format, $root_path);
    
    if (lc($scope) eq 'blog') {
        return { status => ERROR(),
                 message => $app->translate("Unable to upload to blog site path without blog context.") }
        unless ($app->blog);
        ...
        return { status => ERROR(),
                 message => $app->translate("Unable to upload to blog archive path without blog context.") }
        unless ($app->blog);
    ...
    if ( $extra_path =~ m!\.\.|\0|\|! ) {
        return { status => ERROR(),
                 message => $app->translate("Invalid extra path '[_1]'", $extra_path)
        };
    }
    ...
    if ( $extra_path =~ m!^/! ) {
        return { status => ERROR(),
                 message => $app->translate("Extra path must be a relative path '[_1]'", $extra_path)
        };
    }
    ...
    $relative_path = $extra_path ? File::Spec->catfile( $extra_path, $basename ) : $basename;
